...
describe('ConfigLoader', () => {
  ...
  describe('applyDefaults()', () => {
    ...
    it('should override defaults with ~/.okta/okta.yaml file', () => {
      fakeFs.file(path.join(os.homedir(), '.okta', 'okta.yml'), yaml.safeDump({
        client: {
          orgUrl: 'foo'
        }
      }));
    ...
    it('should override ~/.okta/okta.yaml with okta.yaml in the process context', () => {
      fakeFs.file(path.join(process.cwd(), 'okta.yml'), yaml.safeDump({
        client: {
          orgUrl: 'bar'
        }
      }));
...
describe('configLoder.apply()', () => {
  ...
  it('should-deep merge objects, overriding with what is on the right', () => {
