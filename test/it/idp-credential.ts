...
describe('Idp credential API', () => {
  ...
  describe('Key', () => {
    ...
    describe('List keys', () => {
      ...
      afterEach(async () => {
        await client.identityProviderApi.deleteIdentityProviderKey({keyId: key.id});
      });
    ...
    describe('Get key', () => {
      ...
      it('should get key', async () => {
        key = await client.identityProviderApi.getIdentityProviderKey({keyId: key.id});
        expect(key).to.be.exist;
        expect(key).to.be.instanceOf(JsonWebKey);
      });
    });
    ...
    describe('Delete key', () => {
      ...
      it('should delete key', async () => {
        await client.identityProviderApi.deleteIdentityProviderKey({keyId: key.id});
        try {
          await client.identityProviderApi.getIdentityProviderKey({keyId: key.id});
        } catch (e) {
  ...
  describe('Signing CSR', () => {
    ...
    describe('List signing csrs', () => {
      ...
      afterEach(async () => {
        await client.identityProviderApi.revokeCsrForIdentityProvider({idpId: idp.id, csrId: csr.id});
      });
    ...
    describe('Delete signing csr', () => {
      ...
      it('should delete csr', async () => {
        await client.identityProviderApi.revokeCsrForIdentityProvider({idpId: idp.id, csrId: csr.id});
        try {
          csr = await client.identityProviderApi.getCsrForIdentityProvider({idpId: idp.id, csrId: csr.id});
        } catch (e) {
          expect(e.status).to.equal(404);
        }
      });
    ...
    describe('Publish signing csr', () => {
      ...
      it('should publish cert and remove csr (DER base64)', async () => {
        ...
        const key = await client.identityProviderApi.publishCsrForIdentityProvider({idpId: idp.id, csrId: csr.id, body: b64});
        expect(key).to.be.instanceOf(JsonWebKey);
      ...
      it('should publish cert and remove csr (PEM)', async () => {
        ...
        const key = await client.identityProviderApi.publishCsrForIdentityProvider({idpId: idp.id, csrId: csr.id, body: pem});
        expect(key).to.be.instanceOf(JsonWebKey);
        expect(key.n).to.equal(n);
        expect(key.x5c[0]).to.equal(b64);
      ...
      it('should publish cert and remove csr (DER)', async () => {
        ...
        const key = await client.identityProviderApi.publishCsrForIdentityProvider({idpId: idp.id, csrId: csr.id, body: der});
        expect(key).to.be.instanceOf(JsonWebKey);
        expect(key.n).to.equal(n);
        expect(key.x5c[0]).to.equal(b64);
  ...
  describe('Signing Key', () => {
    ...
    describe('Get signing keys', () => {
      ...
      it('should return a Collection', async () => {
        key = await client.identityProviderApi.getIdentityProviderSigningKey({idpId: idp.id, keyId: key.kid});
        expect(key).to.be.exist;
      });
    });
    ...
    describe('Clone key', () => {
      ...
      it('should clone key to another idp', async () => {
        const clonedKey = await client.identityProviderApi.cloneIdentityProviderKey({idpId: idp.id, keyId: key.kid, targetIdpId: anotherIdp.id });
