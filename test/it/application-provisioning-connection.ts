...
describe('Application API: provisioning connection for application', () => {
  ...
  it('provides method for getting default provisioning connection', async () => {
    const provisioningConnection = await client.applicationApi.getDefaultProvisioningConnectionForApplication({appId: application.id});
  ...
  it('provides methods for activating and deactivating default provisioning connection', async () => {
    try {
      const response = await client.applicationApi.activateDefaultProvisioningConnectionForApplication({appId: application.id});
    ...
    const response = await client.applicationApi.deactivateDefaultProvisioningConnectionForApplication({appId: application.id});
    expect(response).to.be.undefined;
    const provisioningConnection = await client.applicationApi.getDefaultProvisioningConnectionForApplication({appId: application.id});
    expect(provisioningConnection.status).to.equal('DISABLED');
  });
  ...
  it('provides method for creating provisioning connection for application', async () => {
    try {
      await client.applicationApi.updateDefaultProvisioningConnectionForApplication({appId: application.id,
