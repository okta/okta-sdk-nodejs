...
The client can be used to fetch collections of resources, in this example we'll use the [Users: List Users][] API.  When fetching collections, you can use the `each()` and `next()` methods to iterate through the collection.  For more information see [Collections](#collections).
...
## Collections
When the client is used to fetch collections of resources, a collection instance is returned.  The collection encapsulates the work of paginating the API to fetch all resources in the collection (see [Pagination][]).  The collection provides two methods, `each()` and `next()`, as described below.
