...
class DefaultRequestExecutor extends RequestExecutor {
  constructor(config = {}) {
    ...
    if (config.maxRetries && config.maxRetries < 0) {
      throw new Error(
        `okta.client.rateLimit.maxRetries provided as ${config.maxRetries} but must be 0 (disabled) or greater than zero`
      );
    }

    if (config.requestTimeout && config.requestTimeout < 0) {
      throw new Error(
        `okta.client.rateLimit.requestTimeout provided as ${config.requestTimeout} but must be 0 (disabled) or greater than zero`
      );
    }
  ...
  buildRetryRequest(request, requestId, delayMs) {
    ...
    newRequest.timeout =
      this.requestTimeout > 0 ? this.requestTimeout - elapsedMs - delayMs : 0;
    ...
    newRequest.headers[this.retryCountHeader] = newRequest.headers[
      this.retryCountHeader
    ]
      ? newRequest.headers[this.retryCountHeader] + 1
      : 1;
    return newRequest;
  }
  ...
  parseResponse(request, response) {
    if (
      response.status === 429 &&
      this.validateRetryResponseHeaders(response) &&
      !this.maxRetriesReached(request)
    ) {
      const elapsedMs = Date.now() - request.startTime;
      ...
      if (this.requestTimeout > 0) {
        ...
        if (delayDelta >= this.requestTimeout) {
          return Promise.reject(
            new Error(
              'HTTP 429 retry delay would exceed okta.client.rateLimit.requestTimeout'
            )
          );
        }
      }
  ...
  maxRetriesReached(request) {
    ...
    const retryCount =
      request.headers && request.headers[this.retryCountHeader];
